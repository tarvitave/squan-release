...
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/tarvitave/squan-release/main/docker-compose.yml" `
    -OutFile "docker-compose.yml"

# Download .env.example
Write-Host "[*] Downloading .env.example..." -ForegroundColor Yellow
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/tarvitave/squan-release/main/.env.example" `
    -OutFile ".env.example"
...
# Prompt for API key
Write-Host ""
Write-Host "[!] You need at least one AI provider API key:" -ForegroundColor Green
...
if ($editNow -eq "y" -or $editNow -eq "Y") {
    notepad .env
    Write-Host ""
    Write-Host "Please add at least one API key and save the file." -ForegroundColor Yellow
...
# Start Squan
Write-Host ""
Write-Host ">>> Starting Squan..." -ForegroundColor Cyan
docker compose up -d

# Show status
Write-Host ""
Write-Host "[OK] Squan is starting!" -ForegroundColor Green
Write-Host ""
Write-Host "    Access Squan at: http://localhost:3000" -ForegroundColor Cyan
Write-Host ""
